...
const EmployeeTaskList = ({ employeeId }: EmployeeTaskListProps) => {
  ...
  const todaysTasks = filteredTasks.filter((t) => isToday(t.created_at) || isToday(t.deadline));
  const otherTasks = filteredTasks.filter((t) => !(isToday(t.created_at) || isToday(t.deadline)));
  ...
  return (
    <div>
      ...
      {todaysTasks.length > 0 && (
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm sm:text-base font-semibold">Today</h3>
            <Badge variant="outline" className="text-xs">{todaysTasks.length}</Badge>
          </div>
          <div className="grid gap-3 sm:gap-4">
            {todaysTasks.map((task) => (
              <Card key={task.id} className="p-4 sm:p-6 hover:shadow-lg transition-all animate-fade-in border-l-4 border-l-primary">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  {task.status === "pending" && (
                    <>
                      <Button
                        size="sm"
                        onClick={() => startTask(task.id, task.is_active)}
      ...
      <div className="grid gap-3 sm:gap-4">
        {otherTasks.map((task) => (
          <Card key={task.id} className="p-4 sm:p-6 hover:shadow-lg transition-all animate-fade-in">
            <div className="flex items-start justify-between mb-3 sm:mb-4">
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                  <h3 className="font-semibold text-sm sm:text-lg truncate">{task.title}</h3>
